...
public class easy {
    ...
    public static int sumSalary(){
        int sum=0;
        for (Employee employee:employees){
            sum+= employee.getSalary((int) (currentSalary * (percent / 100f + 1)));
        }
        return sum;
    }
    public static Employee findPersonWithMinSalary(){
        Employee result=employees[0];
        int minSalary= employees[0].getSalary((int) (currentSalary * (percent / 100f + 1)));
        for(Employee employee : employees){
            if(employee.getSalary((int) (currentSalary * (percent / 100f + 1)))<minSalary){
                minSalary= employee.getSalary((int) (currentSalary * (percent / 100f + 1)));
                result=employee;
            }
        }
        return result;
    ...
    public static Employee findPersonWithMaxSalary(){
        Employee result=employees[0];
        int maxSalary= employees[0].getSalary((int) (currentSalary * (percent / 100f + 1)));
        for(Employee employee : employees){
            if(employee.getSalary((int) (currentSalary * (percent / 100f + 1)))> maxSalary){
                maxSalary= employee.getSalary((int) (currentSalary * (percent / 100f + 1)));
                result=employee;
            }
        }
        return result;
    ...
    public static int averageSalary(){
       return (int) (sumSalary()/(float) employees.length);
    }
